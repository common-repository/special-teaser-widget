...
class STW_Admin extends A5_OptionPage {
	...
	function add_admin_menu() {
		...
		add_plugins_page('Special Teaser Widget '.__('Settings', 'special-teaser-widget'), '<img alt="" src="'.plugins_url('special-teaser-widget/img/a5-icon-11.png').'"> Special Teaser Widget', 'administrator', 'special-teaser-settings', array($this, 'build_options_page'));
	...
	function build_options_page() {
		...
		if (isset($_GET['settings-updated']) && true == $_GET['settings-updated']) :
			...
			switch (self::$action) :	
				...
				case 'delete':
					...
					add_settings_error('stw_options', 'delete-style', __('Style deleted.', 'special-teaser-widget'), 'updated');
		...
		self::open_page('Special Teaser Widget', __('http://wasistlos.waldemarstoffel.com/plugins-fur-wordpress/special-teaser-widget', 'special-teaser-widget'), 'special-teaser-widget', __('Plugin Support', 'special-teaser-widget'));
		...
		self::sortable('top', self::postbox(__('General Settings', 'special-teaser-widget'), 'style-table', 'stw_style_table'));
		
		// Widget Container
		
		self::sortable('upper-middle', self::postbox(__('Widget Container', 'special-teaser-widget'), 'widget-style', 'stw_widget_style'));
		
		self::sortable('middle', self::postbox(__('Widget Title', 'special-teaser-widget'), 'title-style', 'stw_title_style'));
		
		self::sortable('lower-middle', self::postbox(__('Widget Text', 'special-teaser-widget'), 'text-style', 'stw_text_style'));
		
		self::sortable('bottom', self::postbox(__('Widget Image', 'special-teaser-widget'), 'image-style', 'stw_img_style'));
		
		if (WP_DEBUG === true) self::sortable('deep-down', self::debug_info(self::$options, __('Debug Info', 'special-teaser-widget')));
		...
		$donationtext = self::tag_it(__('If you like the plugin and find it useful, you might think of rewarding the work that was spent creating it.', 'special-teaser-widget'), 'p');
		
		self::sortable('side_middle', self::donation_box($donationtext, __('Donations', 'special-teaser-widget'), 'GLAEHEAM7D7ME', 'http%3A%2F%2Fwasistlos.waldemarstoffel.com%2Fplugins-fur-wordpress%2Fspecial-teaser-widget'));
	...
	function initialize_settings() {
		...
		add_settings_field('style_table', __('Styles:', 'special-teaser-widget'), array($this, 'style_table'), 'stw_style_table', 'stw_settings');
		
		add_settings_field('stw_inline', __('Debug:', 'special-teaser-widget'), array($this, 'inline_field'), 'stw_style_table', 'stw_settings', array(__('If you can&#39;t reach the dynamical style sheet, you&#39;ll have to diplay the styles inline. By clicking here you can do so.', 'special-teaser-widget')));
		
		$cachesize = count(self::$options['cache']);
		
		$entry = ($cachesize > 1) ? __('entries', 'special-teaser-widget') : __('entry', 'special-teaser-widget');
		
		if ($cachesize > 0) add_settings_field('stw_reset', sprintf(__('Empty cache (%d %s)', 'special-teaser-widget'), $cachesize, $entry), array($this, 'reset_field'), 'stw_style_table', 'stw_settings', array(__('You can empty the plugin&#39;s cache here, if necessary.', 'special-teaser-widget')));
		
		if ('edit' != self::$action) add_settings_field('style_name', __('Name:', 'special-teaser-widget'), array($this, 'style_name_field'), 'stw_style_table', 'stw_settings', array(__('Give a unique name for the style.', 'special-teaser-widget')));
		
		add_settings_section('stw_settings', false, array($this, 'display_widget_section'), 'stw_widget_style');
		
		add_settings_field('widget_css', __('Main Container:', 'special-teaser-widget'), array($this, 'widget_field'), 'stw_widget_style', 'stw_settings');
		
		add_settings_section('stw_settings', false, array($this, 'display_title_section'), 'stw_title_style');
		
		add_settings_field('title_container_css', __('Title Container:', 'special-teaser-widget'), array($this, 'title_container_field'), 'stw_title_style', 'stw_settings');
		
		add_settings_field('title_font_css', __('Title Font:', 'special-teaser-widget'), array($this, 'title_font_field'), 'stw_title_style', 'stw_settings', array(__('Here you give the css styles for the title of your widget.', 'special-teaser-widget')));
		
		add_settings_field('title_link_css', __('Style for Title Link:', 'special-teaser-widget'), array($this, 'title_link_field'), 'stw_title_style', 'stw_settings', array(__('Enter here the style and hover style for links in the title container.', 'special-teaser-widget')));
		
		add_settings_field('title_link_hover_css', __('Hover Style for Title Link:', 'special-teaser-widget'), array($this, 'title_link_hover_field'), 'stw_title_style', 'stw_settings');
		
		add_settings_section('stw_settings', false, array($this, 'display_text_section'), 'stw_text_style');
		
		add_settings_field('text_container_css', __('Container:', 'special-teaser-widget'), array($this, 'text_container_field'), 'stw_text_style', 'stw_settings');
		
		add_settings_field('text_paragraph_css', __('Paragraph:', 'special-teaser-widget'), array($this, 'text_paragraph_field'), 'stw_text_style', 'stw_settings', array(__('For some themes, the paragraphs in the text container have to be styled differently.', 'special-teaser-widget')));
		
		add_settings_field('text_link_css', __('Style for Text Links:', 'special-teaser-widget'), array($this, 'text_link_field'), 'stw_text_style', 'stw_settings', array(__('Enter here the style and hover style for links in the widget container.', 'special-teaser-widget')));
		
		add_settings_field('text_link_hover_css', __('Hover Style for Text Links:', 'special-teaser-widget'), array($this, 'text_link_hover_field'), 'stw_text_style', 'stw_settings');
		
		add_settings_section('stw_settings', false, array($this, 'display_img_section'), 'stw_img_style');
		
		add_settings_field('img_css', __('Style for Images:', 'special-teaser-widget'), array($this, 'img_field'), 'stw_img_style', 'stw_settings');
		
		add_settings_field('img_hover_css', __('Hover Style for Images:', 'special-teaser-widget'), array($this, 'img_hover_field'), 'stw_img_style', 'stw_settings');
	...
	function display_style_section() {
		...
		self::tag_it(__('You can create and add different styles for the widget here. If there are no styles defined, the widget will not be functioning. The names of the styles will appear in a drop down menu in the widgetsettings.', 'special-teaser-widget'), 'p');
	...
	function style_table() {
		...
		if (!empty($stw_styles)) :
			...
			$thead = self::tag_it(__('Style', 'special-teaser-widget'), 'th', 3, array('style' => 'width: 20%'));
			...
			$output = __('There are no styles defined so far.', 'special-teaser-widget');
	...
	function display_widget_section() {
		...
		self::tag_it(__('Here you give the css styles for the main container of your widget. This is obligatory, all the other styles are optional.', 'special-teaser-widget'), 'p', false, false, true);
	...
	function display_title_section() {
		...
		self::tag_it(__('Here you give the css styles for the title container of your widget.', 'special-teaser-widget'), 'p', false, false, true);
	...
	function display_img_section() {
		...
		self::tag_it(__('There can be done a lot with a bit of CSS3 to your images.', 'special-teaser-widget'), 'p', false, false, true);
	...
	function validate($input) {
		...
		if ($count != 0) :
			...
			if (empty($style[$key]['style_name'])) $error['noname'] = __('Please enter a name for the style.', 'special-teaser-widget');
			...
			if (isset($newkey) && @array_key_exists($newkey, self::$options['style'])) :
				...
				$error['key_exists'] = __('A style with that name already exists.', 'special-teaser-widget');
				
				unset($style[$key]['style_name']);
				
			endif;
			
			if (empty($style[$key]['main_container'])) $error['no_main_style'] = __('Please enter a style for the main container.', 'special-teaser-widget');
		...
		if (isset($input['reset_options'])) :
			...
			add_settings_error('stw_options', 'empty-cache', __('Cache emptied.', 'special-teaser-widget'), 'updated');
	...
	private static function preview_widget($style) {
		...
		echo '<div class="'.$style.'_title"><h3><a href="#">'.__('Widget Title', 'special-teaser-widget').'</a></h3></div>'.$eol;
