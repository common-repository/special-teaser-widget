...
if (!class_exists('A5_Widget')) require_once STW_PATH.'class-lib/A5_WidgetClass.php';
...
class SpecialTeaser {
	...
	function __construct() {
		...
		load_plugin_textdomain('special-teaser-widget', false , basename(dirname(__FILE__)).'/languages');
	...
	function register_links($links, $file) {
		...
		if ($file == STW_BASE) :
			...
			$links[] = '<a href="http://wordpress.org/extend/plugins/special-teaser-widget/faq/" target="_blank">'.__('FAQ', 'special-teaser-widget').'</a>';
			$links[] = '<a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=GLAEHEAM7D7ME" target="_blank">'.__('Donate', 'special-teaser-widget').'</a>';
	...
	function plugin_action_links( $links, $file ) {
		...
		if ($file == STW_BASE) array_unshift($links, '<a href="'.admin_url( 'plugins.php?page=special-teaser-settings' ).'">'.__('Settings', 'special-teaser-widget').'</a>');
